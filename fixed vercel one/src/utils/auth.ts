...
export async function fetchRobloxUser(
  ...
  try {
    ...
    const data = contentType.includes('application/json') ? await res.json() : { error: `Roblox service returned an unexpected response (${res.status}).` };
...
export async function verifyRobloxBio(
  ...
  try {
    ...
    const contentType = res.headers.get('content-type') || '';
    const data = contentType.includes('application/json') ? await res.json() : { error: `Roblox verification returned an unexpected response (${res.status}).` };
    if (!res.ok) {
      return {
        verified: false,
